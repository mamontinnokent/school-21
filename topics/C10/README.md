...
## Теория
- Открытие файлов через `open` и чтение блоками с `read`.
- Обработка ошибок системных вызовов и проверка кодов возврата.
- Вывод содержимого с помощью `write` и буферов.
- Корректное закрытие дескрипторов `close(fd)` во всех ветках.
